...
&НаКлиенте
Перем НачалоВызоваВнешнегоСобытия;
...
Процедура Тест3НаСервере(ИмяФайла)
	...
	УниверсальнаяДата1 = ТекущаяУниверсальнаяДата();
	УниверсальнаяДата2 = ОбъектКомпоненты2.Utc();
	Разница = УниверсальнаяДата2 - УниверсальнаяДата1;
	Если Разница > 1 Или Разница < -1 Тогда
		ВызватьИсключение "Метод Utc() вернул некорректную дату";
	КонецЕсли;
	
	Попытка
		ОбъектКомпоненты2.MethodNoParams();
	Исключение
		ВызватьИсключение "Не удалось вызвать MethodNoParams";
	КонецПопытки;
...
Асинх Процедура ТестВнешнееСобытие(Команда)
	...
	НачалоВызоваВнешнегоСобытия = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Результат = Ждать КомпонентаДляВнешнегоСобытия.CallExternalEventAsync();
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Сообщить(СтрШаблон("Прошло: %1 мс, Источник: %2, Событие: %3, Данные: %4", ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоВызоваВнешнегоСобытия, Источник, Событие, Данные));
	
	Если Событие = "Shutdown" Тогда
		КомпонентаДляВнешнегоСобытия = Неопределено;
		Сообщить("Работа внешней компоненты завершена");
	КонецЕсли;
